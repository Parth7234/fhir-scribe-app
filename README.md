...
The doctor can then **edit** the AI-generated notes inline, **download** a polished prescription PDF, and **review** the complete FHIR R4 JSON bundle. The entire app is secured with **Clerk JWT authentication** — every API endpoint requires a valid token, verified against Clerk's JWKS public keys using RS256. The frontend is deployed on **Vercel** and the backend on **Render**, making the app instantly accessible from any mobile browser without installation.
...
| 🔐 **Clerk Authentication** | Secure doctor sign-in with JWT-based route protection on frontend & backend |
...
```mermaid
graph LR
    A[📱 Mobile Browser] -->|Audio WebM| B[Clerk Auth]
    B -->|JWT Token| C[FastAPI Backend]
...
```
🎤 Audio (WebM) → 📝 Hinglish Transcript → 📋 Structured Notes → 🏥 FHIR R4 Bundle → ✅ Validation
```
...
# Configure environment
# Add VITE_API_URL and VITE_CLERK_PUBLISHABLE_KEY to .env
...
All protected endpoints require a valid `Authorization: Bearer <clerk_jwt>` header.
...
│   ├── src/
│   │   ├── App.tsx             # Main app (recording, notes, FHIR, PDF, auth)
│   │   ├── App.css             # Custom animations & glassmorphism
│   │   ├── index.css           # Tailwind + base styles
│   │   └── main.tsx            # React + Clerk provider entry point
│   ├── package.json
│   ├── vite.config.ts
│   └── tailwind.config.js
...
|-----------|----------|---------|
| **Frontend** | Vercel | Auto-deploy from Git, env vars for API URL & Clerk key |
| **Backend** | Render | FastAPI + Uvicorn, env vars for Gemini API key |
